...
public class Generator {
    ...
    public void generate() {
        ...
        if (choose_parts != null) {
            String[] ss = choose_parts.split(",");
            List<TableMeta> new_meta = new ArrayList<>();
            for (String s : ss) {
                Pattern pattern = Pattern.compile(s, Pattern.CASE_INSENSITIVE);
                for (TableMeta tableMeta : tableMetas) {
                    if (pattern.matcher(tableMeta.name).find()) {
                        new_meta.add(tableMeta);
                    }
                }
            }
            tableMetas = new_meta;
