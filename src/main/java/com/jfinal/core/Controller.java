...
import com.jfinal.ext.zyq.ZBean;
...
public abstract class Controller {
	...
	//自定义方法
	public <T> T getZBean(Class<? extends ZBean> claz, String name) {
		name = name.trim();
		Map<String, Method> setMe = new HashMap<String, Method>();
		try {
			Object object = claz.newInstance();
			for (Method t : claz.getDeclaredMethods()) {
				if (t.getName().startsWith("set")) {
					setMe.put(t.getName().substring(3).trim().toLowerCase(), t);
				}
			}
			Map<String, String[]> parasMap = getRequest().getParameterMap();
			for (Map.Entry<String, String[]> m : parasMap.entrySet()) {
				if (m.getKey().startsWith(name + ".")) {
					Method method = setMe.get(m.getKey().substring(name.length() + 1).toLowerCase());
					Object o = m.getValue()[0];
					if (method.getParameterTypes()[0] == Integer.class) {
						o = Integer.valueOf((String) o);
					}
					method.invoke(object, m.getValue().length == 1 ? o : "");
				}
			}
			return (T) object;
		} catch (Exception e) {
